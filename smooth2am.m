function matrixOut = smooth2am(matrixIn,Nr,Nc)
...
q=matrixOut;
n=size(q,2); %Number of ensembles at that transect
m=size(q,1); %Number of bins at that transect
blank=cell(n, 1); %Creating a blank cell for loop data
for i=1:n
    blank{i}=nanmean(q(:,i)); %Calculating mean for each ensemble ignoring nan and putting that to blank cell
end
blankm=cell2mat(blank); %convert blank to matrix
blankm=blankm'; %transpose
p=zeros(m,n); %creating a zero matrix for next loop
for k=1:n
    for j=1:m
         if isnan(q(j,k))  
            p(j,k)=nan; % if q matrix element is nan ignore that
         else   
            p(j,k)=q(j,k)-blankm(1,k); % % if q matrix element is not nan originally, value-mean
         end
     end
end
matrixOut=p;
